        ...
        sections: [
            {
                ...
                children: [
                    ...
                    ...(() => {
                        ...
                        // CLASSIC TEMPLATE — Logo left + School name & metadata right (compact, side-by-side)
                        if (!metadata.headerTemplate || metadata.headerTemplate === 'classic') {
                            // Build logo cell content
                            const logoCellChildren: any[] = [];
                                    ...
                                    logoCellChildren.push(new Paragraph({
                                        ...
                                        children: [
                                            new ImageRun({
                                                ...
                                                data: bytes,
                                                transformation: {
                                                    width: metadata.schoolLogoWidth || 72,
                                                    height: metadata.schoolLogoHeight || 72
                                                }
                                            })
                                        ],
                            ...
                            // Build info cell — school name + 2-row grid
                            const noBorders = {
                            ...
                            const infoTable = new Table({
                                ...
                                borders: noBorders,
                                rows: [
                                    // Row 1: STD | Exam Name | TIME
                                    new TableRow({
                                        children: [
                                            new TableCell({ borders: noBorders, children: [new Paragraph({ children: [new TextRun({ text: `STD: ${metadata.standard?.toUpperCase() || ''}`, bold: true, size: 22 })] })] }),
                                            new TableCell({ borders: noBorders, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: metadata.examName?.toUpperCase() || '', bold: true, size: 22 })] })] }),
                                            new TableCell({ borders: noBorders, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: `TIME:${metadata.timeAllowed?.toUpperCase() || ''}`, bold: true, size: 22 })] })] }),
                                        ]
                                    }),
                                    // Row 2: DATE | SUBJECT | MARKS
                                    new TableRow({
                                        children: [
                                            new TableCell({ borders: noBorders, children: [new Paragraph({ children: [new TextRun({ text: `DATE:${metadata.date ? new Date(metadata.date).toLocaleDateString() : '__________'}`, bold: true, size: 22 })] })] }),
                                            new TableCell({ borders: noBorders, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `SUBJECT: ${metadata.subject?.toUpperCase() || ''}`, bold: true, size: 22 })] })] }),
                                            new TableCell({ borders: noBorders, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: `MARKS:${metadata.totalMarks}`, bold: true, size: 22 })] })] }),
                                        ]
                                    })
                                ]
                            });

                            const infoCellChildren: any[] = [
                                new Paragraph({
                                    alignment: AlignmentType.CENTER,
                                    children: [new TextRun({ text: metadata.schoolName || '', bold: true, size: (metadata.schoolNameSize || 18) * 2 })],
                                    spacing: { before: 0, after: 40 }
                                }),
                                infoTable
                            ];

                            // Outer side-by-side table: [logo | info]
                                ...
                                rows: [
                                    new TableRow({
                                        children: [
                                            new TableCell({
                                                borders: noBorders,
                                                width: { size: metadata.schoolLogo ? 12 : 0, type: WidthType.PERCENTAGE },
                                                children: logoCellChildren,
                                            }),
                                            new TableCell({
                                                borders: noBorders,
                                                width: { size: metadata.schoolLogo ? 88 : 100, type: WidthType.PERCENTAGE },
                        ...
                        // MODERN TEMPLATE
                            ...
                            headerBlocks.push(new Paragraph({
                                alignment: metadata.schoolNameAlignment === 'right' ? AlignmentType.RIGHT : metadata.schoolNameAlignment === 'center' ? AlignmentType.CENTER : AlignmentType.LEFT,
                                children: [new TextRun({ text: metadata.schoolName || "Institution Name", bold: true, size: (metadata.schoolNameSize || 24) * 2 })],
                            }));
                            headerBlocks.push(new Paragraph({
                                alignment: AlignmentType.LEFT,
                        ...
                        // IVY LEAGUE TEMPLATE
                            ...
                            headerBlocks.push(new Paragraph({
                                alignment: metadata.schoolNameAlignment === 'left' ? AlignmentType.LEFT : metadata.schoolNameAlignment === 'right' ? AlignmentType.RIGHT : AlignmentType.CENTER,
                                children: [new TextRun({ text: metadata.schoolName?.toUpperCase() || "INSTITUTION NAME", bold: true, size: (metadata.schoolNameSize || 24) * 2, font: "Times New Roman" })],
                                spacing: { after: 100 }
                            }));
                            headerBlocks.push(new Paragraph({
                                alignment: AlignmentType.CENTER,
                        ...
                        // MINIMALIST TEMPLATE
                            ...
                            headerBlocks.push(new Paragraph({
                                alignment: metadata.schoolNameAlignment === 'right' ? AlignmentType.RIGHT : metadata.schoolNameAlignment === 'center' ? AlignmentType.CENTER : AlignmentType.LEFT,
                                children: [new TextRun({ text: metadata.schoolName?.toUpperCase() || "INSTITUTION NAME", size: (metadata.schoolNameSize || 24) * 2, color: "777777" })],
                                spacing: { after: 50 }
                            }));
                            headerBlocks.push(new Paragraph({
                                alignment: AlignmentType.LEFT,
                    ...
                    ...questions.flatMap((q, i) => {
                        ...
                        if (q.type === "MCQ") {
                            ...
                            if (useGrid) {
                                ...
                                for (let i = 0; i < optionsData.length; i += 2) {
                                    ...
                                    const opt1IsCorrect = metadata.showAnswerKey === true && q.content.correctIndex === i;
                                    ...
                                    const opt2IsCorrect = metadata.showAnswerKey === true && q.content.correctIndex === i + 1;
                                ...
                                children.push(new Paragraph({
                                    children: optionsData.map((opt: string, optI: number) => {
                                        const isCorrect = metadata.showAnswerKey === true && q.content.correctIndex === optI;
                                        return new TextRun({
                        ...
                        if (metadata.showAnswerKey === true && q.content.solutionText) {
                            children.push(new Paragraph({
                                children: [
                        ...
                        } else if (metadata.showAnswerKey === true && q.type === "TF" && typeof q.content.isTrue === 'boolean') {
                            children.push(new Paragraph({
                                children: [
                        ...
                        } else if (metadata.showAnswerKey === true && q.type === "MCQ" && typeof q.content.correctIndex === 'number' && q.content.options && q.content.options[q.content.correctIndex]) {
                            children.push(new Paragraph({
                                children: [
